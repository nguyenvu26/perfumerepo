...
export default function StaffDashboard() {
    ...
    const fetchTrend = useCallback(async (p: 'today' | 'week' | 'month' | 'year' | 'quarter' | 'custom') => {
        try {
            setTrendLoading(true);
            const { data } = await api.get<SalesTrendPoint[]>('/staff/reports/analytics/sales-trend', { params: { period: p === 'custom' ? 'month' : p } });
            setTrend(data);
        } catch (e) {
