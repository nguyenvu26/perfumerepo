...
export function CreateReturnModal({
  ...
  const updateReason = (id: number, value: string) =>
    setItemStates((prev) => ({ ...prev, [id]: { ...prev[id], reason: value } }));
  ...
  const handleImageUpload = async (id: number, files: FileList | null) => {
    ...
    setItemStates((prev) => ({ ...prev, [id]: { ...prev[id], uploadingImages: true } }));
    try {
  ...
  const handleSubmit = async () => {
    ...
    if (!bankName.trim() || !bankAccount.trim() || !bankHolder.trim()) {
      toast.error("Vui lòng cung cấp đầy đủ thông tin tài khoản ngân hàng để nhận hoàn tiền");
      return;
    }
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-8 py-6 space-y-6">
                    ...
                    >
                      ...
                      <div
                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                          state.selected ? "bg-gold border-gold" : "border-border"
                        }`}
                      >
                        {state.selected && (
                          <CheckCircle size={12} className="text-primary-foreground fill-primary-foreground" />
                        )}
                      </div>
                      ...
                      <div className="px-4 pb-4 space-y-5 border-t border-gold/10 pt-4">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2 bg-secondary/50 rounded-xl p-1">
                            ...
                            <span className="w-8 text-center font-bold text-sm">{state.quantity}</span>
                            <button
                              onClick={(e) => { e.stopPropagation(); updateQty(item.id, 1); }}
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            {/* Progress indicator */}
                            <span className={`text-[9px] font-bold uppercase tracking-widest ${hasMinImages ? "text-emerald-500" : "text-amber-500"}`}>
                              {imgCount}/{MAX_IMAGES}
                              {!hasMinImages && ` (min ${MIN_IMAGES})`}
                            </span>
                          </div>
                          <p className="text-[9px] text-muted-foreground/70 mb-3">{t("images_desc")}</p>
                          ...
                          <div className="h-1 bg-secondary rounded-full mb-3 overflow-hidden">
                            <div
                              ...
                              style={{ width: `${(imgCount / MAX_IMAGES) * 100}%` }}
                            />
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {state.images.map((url, idx) => (
                              <div key={idx} className="relative w-20 h-20 rounded-xl overflow-hidden group border border-border">
                                ...
                                {/* Overlay with remove */}
                                <button
                                  onClick={(e) => { e.stopPropagation(); removeImage(item.id, idx); }}
                            ...
                            {state.images.length < MAX_IMAGES && (
                              <button
                                onClick={(e) => { e.stopPropagation(); imageInputRefs.current[item.id]?.click(); }}
                              ...
                              >
                                {state.uploadingImages ? (
                                  ...
                                  <>
                                    <ImageIcon size={16} />
                                    <span className="text-[8px] font-bold uppercase">{t("add_photo")}</span>
                                  </>
                                )}
                              </button>
                            )}
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-muted-foreground/70 mb-3">{t("video_desc")}</p>
                          ...
                          {state.videoUrl ? (
                            ...
                            <div className="relative rounded-2xl overflow-hidden border border-border bg-black group">
                              ...
                              {/* Remove button */}
                              <button
                                onClick={(e) => { e.stopPropagation(); removeVideo(item.id); }}
                            ...
                            /* Upload button */
                            <button
                              onClick={(e) => { e.stopPropagation(); videoInputRefs.current[item.id]?.click(); }}
                              ...
                              {state.uploadingVideo ? (
                                ...
                                <>
                                  ...
                                  <span className="text-[8px] text-muted-foreground/60 uppercase tracking-widest">
                                    MP4 / MOV / WEBM · {MIN_VIDEO_SECONDS}s – {MAX_VIDEO_SECONDS}s
                                  </span>
                                </>
                              )}
                            </button>
                          ...
                          <input
                            ref={(el) => { videoInputRefs.current[item.id] = el; }}
                            type="file"
                            accept="video/*"
                            className="hidden"
                            onChange={(e) => handleVideoSelect(item.id, e.target.files)}
                          />
                        </div>
                      </div>
                    )}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-secondary/30 border border-border rounded-2xl px-4 py-3 text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:border-gold/50 transition-colors resize-none"
            />
            ...
            <div className="grid grid-cols-1 gap-4 bg-gold/5 p-5 rounded-2xl border border-gold/20 mb-6">
              <p className="text-xs text-gold mb-2">
                * Đơn hàng thanh toán theo hình thức online sẽ được hoàn tiền thủ công qua tài khoản ngân hàng.
              </p>
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-1">Tên Ngân hàng</label>
                <input
                  type="text"
                  value={bankName}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-1">Số Tài Khoản</label>
                  <input
                    type="text"
                    value={bankAccount}
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-1">Chủ Tài Khoản</label>
                  <input
                    type="text"
                    value={bankHolder}
